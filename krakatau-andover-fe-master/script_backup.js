const x_data = () => {
	return {
		...
		async waypointCaptureSurfaceAuto() {
			try {
				// Ambil waypoint aktif sekarang dari data kendaraan
				const currentWP = this.vehicleData.current_wp;

				// Ambil daftar waypoint input user yang disimpan
				const waypoints = this.surfaceCamera.waypoints;

				// Jika belum ada waypoint, langsung keluar
				if (!waypoints || waypoints.length === 0) return;

				// Cek apakah currentWP cocok dengan salah satu input user
				if (waypoints.includes(String(currentWP))) {
					...
					// Hapus agar tidak ambil berulang
